...
const Cart = {
  ...
  getCart: function () {
    const cart = localStorage.getItem("kopi_kenangan_cart");
    return cart ? JSON.parse(cart) : [];
  },

  // Save cart to localStorage
  saveCart: function (cart) {
    localStorage.setItem("kopi_kenangan_cart", JSON.stringify(cart));
    ...
    if (cart.length === 0) {
      cartItemsContainer.innerHTML = `
                <div class="cart-empty">
                    ...
                    <button class="btn btn-primary" onclick="Cart.closeCartModal()">Mulai Belanja</button>
                </div>
            `;
  ...
  // Clear cart
  clearCart: function () {
    localStorage.removeItem("kopi_kenangan_cart");
    this.updateCartCount();
    this.updateCartModal();
  },
